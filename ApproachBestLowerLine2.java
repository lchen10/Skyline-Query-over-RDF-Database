...
public class ApproachBestLowerLine2 {
	...
	public static void main(String[] args) throws SQLException {
		...
		try {
			ArrayList<Tuple> candidateList = ApproachBestLowerLine2.RDFJoin(
	...
	private static ArrayList<Tuple> RDFJoin(String[] skylineAttr,
		...
		while (!stop) {
			...
			for (int i = 0; i < skylineAttr.length; i++) {
				...
				// System.out.print("found current tuple: " + foundtuple);
			...
			if (!stop) {
				...
				for (int index = 0; index < skylineAttr.length; index++) {
					if (colBetterTimes[index] == skylineAttr.length) {
						isUpdate = true;
						int worst = (Integer) currentTuples[0].getValue(index); 
